...
* `TurnMemory` tidak menggantikan L0–L3 — ia melengkapi. L1 kawai sudah ada (`logic/memory.rs`: CRUD manual + `memory_extract` cloud one-shot dengan dedup judul); L2–L3 menyusul di atas fondasi yang sama, dan extraction kini membaca `sessions/messages` (persis saran awal) — bukan mengganti `session_artifacts`. Keduanya hidup berdampingan: TurnMemory = bukti proses sesi ini, L1–L3 = pengetahuan terdistilasi lintas sesi. Gap vs Tencent yang tersisa: dedup vektor (bukan judul exact), auto-recall per turn, dan L2/L3 pipeline.
