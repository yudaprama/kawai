...
| **7** | `src-tauri/src/logic/skills.rs` `skill_create/list/get/update/delete` + `prompt_block()` | **Skills tier** — SKILL.md CRUD (unique name, version counter bump per update, `skl-` base62 CSPRNG id) + bounded persona injection into `agent_chat` (per-skill 4k / total 12k char caps; no vectors — static prompt block, not retrieval) | `name, description, content` → `Skill{id,name,description,content,version,…}`; `user_id` → `<skills>` prompt block | `skills` (unique name idx) — `migrations/0008_skills.sql` | ungated (plain libsql) | `commands.rs` `skill_*` / `web.rs` `/api/skill_*` <br>inject: `agent.rs` persona + `skills::prompt_block` |
| **8** | `src-tauri/src/logic/memory.rs` `memory_create/list/update/delete` + `memory_extract()` | **L1 memories** — atomic long-term items (kind `preference/rule/event/fact/goal`); extraction = transcript tail 24k chars → `RemoteLlm` one-shot → JSON array → title dedup (case-insensitive) → insert with `source_session_id`; manual CRUD always offline-safe | `session_id` → `Vec<MemoryItem>`; CRUD `kind,title,content` | `memories` (+ updated/source idx) — `migrations/0009_memories.sql` | ungated CRUD; extraction needs vault (`remote.rs`) | `commands.rs` `memory_*` / `web.rs` `/api/memory_*` <br>UI: Memory page L1 tab |
...
| **Memori berlapis** | L0 = `sessions/messages`; L1 = `memories` table + `memory_extract` (transcript → `RemoteLlm` → JSON → dedup judul) + `memory::prompt_block()` persona injection `logic/memory.rs` — tanpa vec0/LLM-dedup; L2/L3 belum | L0 JSONL daily+vec0/FTS5, L1 JSONL+vec0/TCVDB+LLM dedup `l1-dedup.ts`, L2 `scene_blocks/*.md`, L3 `persona.md` `TECHNOLOGIES.md:379-502` | ⚠️ L0+L1 dasar | Ekstraksi L1 kawai = cloud one-shot + dedup judul exact (bukan vector top-K + LLM conflict); injection kini otomatis per-session via `<memories>` prompt block (800 char/entry · 4k total · 24 items max), recall selektif (vector search) belum — semua L1 ter-inject. Mode `keyword/semantic/hybrid` 1:1 (`rag.rs:mode` ↔ `TECHNOLOGIES.md:324-334`). |
| **Skills** | `skills` table + `skill_*` CRUD + `prompt_block()` persona injection (`logic/skills.rs`) — no vectors yet | SQLite `skills`+`skill_vec(vec0)`+FTS5, RRF 3-mode, version chain TTL `TECHNOLOGIES.md:244-334` | ⚠️ Dasar ada | CRUD + version bump + injection shipped; vec0/FTS5 hybrid search & resource files belum — pola `rag.rs` bisa dipakai saat dibutuhkan. |
...
* `TurnMemory` tidak menggantikan L0–L3 — ia melengkapi. L1 kawai sudah ada (`logic/memory.rs`: CRUD manual + `memory_extract` cloud one-shot dengan dedup judul + `memory::prompt_block()` injection ke persona agent per-session, 800 char/entry · 4k total · 24 items max); L2–L3 menyusul di atas fondasi yang sama, dan extraction kini membaca `sessions/messages` (persis saran awal) — bukan mengganti `session_artifacts`. Keduanya hidup berdampingan: TurnMemory = bukti proses sesi ini, L1 = pengetahuan terdistilasi lintas sesi yang ter-inject otomatis. Gap vs Tencent yang tersisa: dedup vektor (bukan judul exact), recall selektif via vector search (kawai inject semua L1), dan L2/L3 pipeline.
* Jangan indeks `session_artifacts.content` ke `rag_chunks` secara otomatis — sudah ada `rag Files` untuk dokumen; TurnMemory sengaja tidak di-embed agar tidak mencemari ruang vektor dokumen dengan output tool sementara. `memories` (L1) juga tidak di-embed di tier ini — bila recall semantik dibutuhkan, tambahkan `vec0`/FTS5 mengikuti pola `rag.rs`, jangan campur tabelnya.
