...
3. **`local_llm_smoke` streaming regression gate.** ✅ Done 2026-08-20: `.github/workflows/ci.yml` runs a `macos-smoke` job (`bazel` cache → build dylib → `scripts/bundle-litert-dylibs.sh` → `cargo test --features litert,office --lib` → `local_llm_smoke` (LFM2.5-1.2B int8 fixture, cached — gemma-3-270m went gated on HF) / `remote_smoke` / `draft_smoke` / `agent_eval`) plus a `web` job (ubuntu: `bun install` + `bun run build` + `cargo check --features web`) and a `linux-check` job (`cargo check --features litert,office`). `agent_smoke` stays skipped (interactive cloud state).
