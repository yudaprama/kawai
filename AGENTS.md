...
src-tauri/src/logic/scrape.rs    # web read + search tiering (office): web_read tool — on-device webview engine (injected trait) → Cloudflare /markdown fallback; web_search tool — Bing SERP over the same chain (DOM extractor on tier 0, markdown-link parse on tier 1); challenge detection, daily budgets, LRU cache
src-tauri/src/webview_engine.rs  # tauri-side WebViewFetch: hidden WebviewWindow + eval_with_callback extractor (office feature; registered in lib.rs, never in kawai-web)
...
rig-components/binance/          # Binance agent tools (hand-written, feature "binance"): keyless public spot market data via crates.io binance-sdk (rustls-tls + spot only) + in-process TA over ta (binance_price / binance_depth / binance_klines / binance_ta_analyze); vendored binance-connector-rust/ is reference-only
...
KAWAI_BINANCE_REST_BASE=https://data-api.binance.vision  # optional REST base override; default = api.binance.com (451 geo-blocks some hosting regions — the mirror is the market-data-only endpoint). CI smoke sets it.
# ── Web read tiering — Cloudflare fallback budgets (logic/scrape.rs) ──
...
3. **`local_llm_smoke` streaming regression gate.** ✅ Done 2026-08-20: `.github/workflows/ci.yml` runs `macos-smoke`, `linux-smoke`, and `windows-smoke` jobs (each bazel-builds the LiteRT-LM C engine natively, then runs `cargo test --features litert,office --lib` → `local_llm_smoke` (LFM2.5-1.2B int8 fixture, cached) / `remote_smoke` / `draft_smoke` / `agent_eval`), plus a `web` job (ubuntu: `bun install` + `bun run build` + `cargo check --features web`) and a `linux-check` job (`cargo check --features litert,office`). `agent_smoke` stays skipped (interactive cloud state).
