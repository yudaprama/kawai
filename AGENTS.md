...
- **Two `reqwest` versions coexist** (0.12 direct + jigsawstack; 0.13 via `youtube_transcript`, office-gated). Expected — rig-core 0.42 already forces 0.13 into the graph. fastembed/ONNX (`ort-sys`) was previously a blocker for `cargo check --target aarch64-apple-ios --features office` — it is now `#[cfg]`-gated out on Android/iOS (`kawai-embedding/Cargo.toml`), so mobile builds no longer pull it.
...
src-tauri/src/logic.rs           # PURE logic; rig + libsql + local_llm (litert) + db token minting + ensure_model (auto-download from HF Hub)
