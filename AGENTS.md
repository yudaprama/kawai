...
│   ├── memory/ (kawai-memory)     # L1 memories CRUD (preference/rule/event/fact/goal, mem-* base62) + hybrid memory_search (vector+BM25/RRF) + memory_graph_search (entity mentions) + memory_extract + memory_consolidate via remote-llm + importance scoring + prompt_block 800/4k/24
