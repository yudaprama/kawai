...
- **CodeGraph**: surgical code context — `crates/toolsets/codegraph` (`codegraph_explore`/`codegraph_status` AgentTools, 15m LRU + single-flight + 12/min budget) via `codegraph` sidecar (`codegraph explore --json`, `CODEGRAPH_BIN` override); wired into every agent via `agent_registry.rs` when `litert` + `codegraph` are on. Frontend `CodeAssetPage` (status + explore input + result view); Tauri `codegraph_explore/status/is_available` + Axum `/api/codegraph_*` (both wrappers, auth at edge). Zero cost when feature off.
