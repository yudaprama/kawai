...
- **LLM (remote)**: hand-rolled OpenAI-compatible SSE client in `logic/remote.rs` (reqwest POST `/chat/completions`, stream parser) — provider pool with health-aware failover from kawai-vault keys (zai → venice → opencode → openrouter → ollama). **Decision (2026-08-16): local Gemma 4 via LiteRT-LM is the orchestrator** — no *replacement* remote provider planned. **The hybrid cloud-subagent tier (2026-08-20)** — `agent_chat` keeps local Gemma 4 as the permanent orchestrator and delegates heavy synthesis to cloud subagent *tools* (`deep_write`, `draft_document`) via prompt-based tool calling whenever a remote LLM is configured (default `zai`, key from kawai-vault; `logic/remote.rs` + config block below). The agent tier runs on local Gemma 4 with prompt-based tool calling (the LiteRT-LM Conversation API has no native function calling). No vault keys ⇒ pool empty ⇒ agents behave pure-local.
