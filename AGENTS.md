...
5. **No AI SDK.** The chat state is produced by `features/chat/hooks/use-supervisor-plan.ts` from raw Tauri Supervisor stream events; the UIMessage/part shapes in `lib/ai-types.ts` are a LOCAL type contract only (field names stay AI-SDK-v5-compatible so the vendored ai-elements components work unmodified). Never add a runtime dep on `ai` / `@ai-sdk/*`.
...
- **Supervisor execution (the product's core)**: every user submission goes goal → `plan_task` (LLM writes a `TaskPlan` against the `ToolRegistry` catalog) → validated plan → `execute_supervisor_plan` (deterministic `kawai-router` scheduler: waves, retries, timeouts, `onError`, typed artifacts, confirmation gates). `src-tauri/src/supervisor.rs` is the composition root: it converts the office `ToolSet` into `ToolMeta` entries and dispatches steps via `ToolSet::execute`. Progress streams as `SupervisorEvent`s to the React frontend (`features/chat/hooks/use-supervisor-plan.ts`), which keeps plan state as the source of truth (rendered by `features/chat/components/plan-progress-panel.tsx`) and persists a structured plan record (goal + per-step status + final output) to session history; the conversation carries only the goal and final output. Step outputs on the wire are bounded — `stepCompleted` carries a ≤2000-char preview; full bodies live in the scheduler, the `ExecutionMemo`, and `supervisor_step_results` (resume needs them). Failure handling: `StepFailed` events carry a failure kind (`timeout`/`confirmation`/`cancelled`/`tool`); a non-user-decided plan failure triggers a **failure-driven replan** — the planner is re-asked with the execution report to produce a revised plan (budget 1 revision, same validation contract); re-executing the **same plan** (Resume button) skips already-completed steps via the persisted `supervisor_step_results` cache (migration 0015, keyed by plan-JSON hash, feeding the `ExecutionMemo`). Three-pane UI (assets rail / chat+canvas / sessions) with a live plan-progress panel on the frontend. Tool registries default to `auto` — a merged catalog of every available domain toolset (office, presentation, analytics, Binance, finance stock/social; first-wins per tool name) — so the planner picks tools across domains; an explicit agent id is an optional narrowing hint. The legacy prompt-based `agent_chat` transport (local Gemma 4 orchestrator loop) was dismantled: its Tauri command, Axum handler, and frontend hook are gone. The planner requires a configured remote LLM pool. The planner call rides a `<user-context>` block — `<persona>` (L3) + goal-relevant `<memories>` (relevance-ranked via `prompt_block_relevant`, which bumps access counters) + the `<skills>` block; each degrades to empty on failure (`render_planner_context`). Memory recall is a first-class tool for every agent: `memory_search` (hybrid semantic) and `memory_graph_search` (entity lookup) + `memory_graph_export` (whole-graph nodes/edges for the Memory page's Graph tab — SVG force-graph adapted from OpenHuman, lazy code-split) are registered in `add_runtime_tools`, bound to `user_id` at build time. The planner runs a **bounded tool-search loop** (mode A, no full-catalog fallback): it sees no catalog — only a core whitelist (web_search, memory_search, artifact_recall, deep_write, draft_document) — and discovers tools via ≤2 search rounds against the Turso-backed tool catalog (`crates/foundation/tool-catalog`, embedded replica + vector/BM25/RRF), then emits the plan validated against the full registry (structure, dispatch keys, confirmation policy, and per-step arguments against each tool's `input_schema` — lightweight JSON-Schema subset, fail-fast before execution); 1 corrective round on validation failure, hard cap 6 LLM calls. Design + benchmark: `PLAN-planner-search-loop.md`.
