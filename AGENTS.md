...
## 🚨 RESOURCE — NEVER RUN HUGE-RESOURCE PROCESSES 🚨

***NEVER run processes that consume huge resources (heavy builds, long-running inference, large test suites, bazel/cargo jobs that would pin the CPU for minutes, GB-scale downloads, etc.) — the dev machine cannot handle them.*** If a task needs such a process, NOTIFY THE USER that it needs to be run manually and wait; do not start it yourself.
