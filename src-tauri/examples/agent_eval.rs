...
const ANALYTICS_TOOLS: &str = r#"[
  ...
  {"type":"function","function":{"name":"data_chart","description":"Render a chart (bar/line/point/area/histogram/pie) from a stored tabular file and save it as an svg the user sees rendered. Takes the same filters/groupBy/aggregations/sortBy as data_query, plus mark, x (category/time column; numeric for histogram), y (numeric column or aggregation alias — omit for histogram, which counts rows itself; required for pie — the slice value), optional color (grouping column; omit for pie), stack (stacked/normalized/grouped — bar/area composition of the color series) and title. x and y must appear in the query result — for aggregates use the group_by column as x and the aggregation alias as y. Pie: one row per category (aggregate groupBy+sum first; ≤20 slices).","parameters":{"type":"object","properties":{"fileId":{"type":"string"},"sheet":{"type":"string"},"mark":{"type":"string","enum":["bar","line","point","area","histogram","pie"]},"x":{"type":"string"},"y":{"type":"string"},"color":{"type":"string"},"stack":{"type":"string","enum":["stacked","normalized","grouped"]},"title":{"type":"string"},"filters":{"type":"array","items":{"type":"object","properties":{"column":{"type":"string"},"operator":{"type":"string","enum":["eq","neq","gt","gte","lt","lte","contains"]},"value":{"type":"string"}},"required":["column","operator","value"]}},"groupBy":{"type":"array","items":{"type":"string"}},"aggregations":{"type":"array","items":{"type":"object","properties":{"column":{"type":"string"},"function":{"type":"string","enum":["sum","avg","min","max","count","count_distinct"]},"alias":{"type":"string"}},"required":["column","function","alias"]}},"sortBy":{"type":"string"},"descending":{"type":"boolean"},"limit":{"type":"integer"}},"required":["fileId","mark","x"]}}},
