...
- Chart requests (\"show/graph/plot/visualize ...\") → data_chart: same filters/groupBy/aggregations/sortBy as data_query plus mark (\"bar\" for comparisons, \"line\" for trends, \"point\" for relationships, \"area\" for cumulative volume, \"histogram\" for the distribution of one numeric column — omit y there), x (category/date column), y (numeric column — an aggregation alias when aggregated), optional color (grouping column) and stack (\"stacked\"/\"normalized\"/\"grouped\" — how bar/area composes the color series). It saves an svg the user sees rendered — afterwards explain the key takeaways in words; never re-dump the charted numbers as a table.\n\
