...
- Product: **an AI agents app** — a catalog of specialized agents; each agent = LLM persona + curated toolset from domain crates, composed through `AgentDefinition` tool builders. UI: the **Kawai Workbench** — landing goal-composer hero, then a two-pane run view (left sidebar: progress rail with the goal composer pinned below | deliverable viewer; post-run the composer becomes a follow-up composer — chips + opt-in previous-deliverable quote, PLAN-followup-composer.md).
...
│  auth.rs  : session type (email string) + dotenv loader     │
├───────────────────────────────────────────────────────────┤
...
## Kawai wallet (Monad device hot-wallet, `monad` feature)

In-app EVM wallet over Monad — `crates/integrations/monad` + `src-tauri/src/logic/monad_wallet.rs` (logic), `frontend/src/features/wallet/` (UI: Assets rail → KAWAI Wallet):

1. **Keys** — one device-scoped hot wallet. The CSPRNG key is generated in-process and persists ONLY in the OS keychain (`monad-wallet/device`); it never reaches the frontend or disk. Signing happens in the backend: the frontend sends plaintext and receives the 65-byte EIP-191 `personal_sign` signature. `monad_wallet_create` is idempotent; `monad_wallet_delete` permanently drops the key (address and funds become unrecoverable locally).
2. **Read chain** (both wrappers) — `get_token_balance` / `get_token_info` (ERC-20 `eth_call`), `estimate_gas`, `get_transaction_receipt`, `check_monad_balance`, `monad_chain_status` — plain JSON-RPC over alloy's HTTP provider. On web only `get_transaction_receipt` is auth-gated (**protected** router, same boundary as fund-moving); the rest are public chain reads.
3. **Fund-moving** (both wrappers; on web these routes sit on the **protected** router behind `auth_middleware`) — `transfer_native` (plain value tx), `transfer_token` / `transfer_usdt` (ERC-20 `transfer`), `deposit_to_vault` (`approve` then `PaymentVault.deposit`). Every successful broadcast is also recorded device-side in a local `wallet_history.json` (newest-first, cap 200, atomic write) served by `monad_wallet_history` (both wrappers; protected on web) — the wallet page's Recent Activity list; on-chain truth stays the explorer. The worker's `POST /transfer` (payout/faucet) signs with a SEPARATE server hot wallet (`PRIVATE_KEY`, mainnet `MONAD_RPC_URL`) — it never touches the user's device key.
4. **Network** — active network + contract addresses live in `src-tauri/src/logic/monad_contracts.rs` (`TESTNET = true` today — Monad testnet, chain 10143, Round 8 deployment), mirrored by `frontend/src/features/wallet/lib/network-config.ts` and the worker's `KAWAI_TOKEN_ADDRESS`. **`NETWORKS.md` at the repo root is the source of truth** — its "Mirror points" list names every file to keep in sync. The `monad` feature is opt-in: release CI builds a second `kawai-mono` variant with `--features litert,tts,binance,monad`; without the feature the ops serve guidance stubs.
5. **Verification** — `src-tauri/examples/monad_wallet_smoke.rs` exercises the exact logic ops the wallet page calls against the live network: read-only probes (chain id, USDT/KAWAI metadata, gas, vault balances, device history log) run as a CI gate (linux-smoke); `--with-wallet` (keychain create/sign roundtrip, never deletes a pre-existing wallet) and `--self-transfer` (1-wei testnet broadcast + receipt poll + history-log assertion) are local opt-in flags.
...
2. **`auth.rs`** — pure auth. `Session` holds the in-process identity for desktop/mobile (the signed-in email string); web carries it in the `kawai_session` cookie. Login/duplicate-email rules live in `kawai-auth`. No transport imports.
...
- **`reqwest`** — remote tier transport: the OpenAI-compatible SSE client in `crates/foundation/remote-llm` (provider pool, health-aware failover). rustls-only. Local Gemma 4 via LiteRT-LM is the model (decision 2026-08-16); remote providers are optional configuration, not a requirement.
...
- **`jsonwebtoken`** — EdDSA only (future sqld token mint in `logic.rs`).
