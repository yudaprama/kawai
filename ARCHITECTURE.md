...
2. **Read chain** (both wrappers) — `get_token_balance` / `get_token_info` (ERC-20 `eth_call`), `estimate_gas`, `get_transaction_receipt`, `check_monad_balance`, `monad_chain_status` — plain JSON-RPC over alloy's HTTP provider.
3. **Fund-moving** (both wrappers; on web these routes sit on the **protected** router behind `auth_middleware`) — `transfer_native` (plain value tx), `transfer_token` / `transfer_usdt` (ERC-20 `transfer`), `deposit_to_vault` (`approve` then `PaymentVault.deposit`). The worker's `POST /transfer` (payout/faucet) signs with a SEPARATE server hot wallet (`PRIVATE_KEY`, mainnet `MONAD_RPC_URL`) — it never touches the user's device key.
...
5. **Verification** — `src-tauri/examples/monad_wallet_smoke.rs` exercises the exact logic ops the wallet page calls against the live network: read-only probes (chain id, USDT/KAWAI metadata, gas, vault balances) run as a CI gate (linux-smoke); `--with-wallet` (keychain create/sign roundtrip, never deletes a pre-existing wallet) and `--self-transfer` (1-wei testnet broadcast + receipt poll) are local opt-in flags.
