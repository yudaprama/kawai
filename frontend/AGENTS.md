...
- **Every submission routes through the Supervisor.** The Workbench composer calls `planAndRun` (session is ensured lazily first). There is exactly one path — the legacy `agent_chat` engine loop, command, handler, and `AgentChatEvent` are fully removed, and there is no chat surface: results render only in the Workbench deliverable viewer. Before that path runs, `use-workbench.run()` enforces the Fase 0a token pre-check (PLAN-qris-topup): ONE `topup_balance` read per submit attempt — tokens ≤ 0 blocks the run (toast "Token habis — isi ulang lewat Top Up" + opens the Top Up page via `emitOpenTopup`), and the read publishes into the shared balance store (`features/topup/use-token-balance.ts`). ANY fetch error fails CLOSED: an unreadable balance blocks the submit with a "Saldo tidak terbaca — coba lagi" toast, mirroring the fail-closed server gate in `supervisor::plan_task`.
...
- **Session management is lazy.** A session is created on the first user message via `ensureSession()`. The title is seeded from the first message (first 80 chars) and generated server-side. Deletes are deferred and batch-capable: `use-chat-sessions.deleteSessions(ids)` removes rows optimistically and fires `delete_chat_session` per id only after a 5s window (one sonner Undo toast covers the whole batch; Undo cancels the calls and reloads server truth). Archive/restore generalizes the same way — `setSessionsArchived(ids, archived)`, optimistic move + per-id server reconcile. The switcher dialog (`session-history-dialog`) owns a keyboard cursor — arrows/Home/End move a highlight over the flat row list (groups + archive), Enter opens (toggles the row in Select mode); hover moves the same cursor. Search is two-layer: an instant local title filter plus a 250ms-debounced server `list_chat_sessions` `query` over titles + message content (server error/older backend falls back to the local layer). The header's **Select** button turns rows into checkboxes behind a bulk bar (Archive/Restore enabled per selection side, Delete, Cancel). Every row (active + archived) exports its transcript through `export_deliverable` as a stored `.md` (slugified title) and opens the file preview on success. On session load the hook best-effort runs `archive_stale_sessions` (30+ idle days → archive) before the first list read.
- **Landing recents ride `list_recent_runs`.** `recent-runs.tsx` lists the newest persisted plan record per session when the session has no in-memory runs; a pick sets `workbench.sessionId` and queues `restored-<rowId>` — the use-workbench restore effect rehydrates the session's records (timestamps = the message row's write time) and the queued pick then opens that run's report. `reloadKey` refetches after a run lands or the sessions dialog closes.
