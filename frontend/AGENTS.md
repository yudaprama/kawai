...
- **Auth bootstrap is in use-supervisor-chat.ts.** The hook first tries `whoami`, then falls back to `set_session` with a dev token. This is the MVP dev-bypass — no Supabase UI is wired in the React frontend.
