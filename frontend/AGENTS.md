...
- **Session management is lazy.** A session is created on the first user message via `ensureSession()`. The title is seeded from the first message (first 80 chars) and generated server-side. Deletes are deferred: `use-chat-sessions.deleteSession` removes the row optimistically and fires `delete_chat_session` only after a 5s window (sonner Undo toast); Undo cancels the call and reloads server truth. The switcher dialog (`session-history-dialog`) owns a keyboard cursor — arrows/Home/End move a highlight over the flat row list (groups + archive), Enter opens; hover moves the same cursor.
