...
import { CmcFearGreedView, CmcOhlcvView, CmcView } from "./cmc-views";
import {
  MonadChainStatusView,
  MonadGasView,
  MonadLogsView,
  MonadTokenBalanceView,
  MonadTokenInfoView,
  MonadTxReceiptView,
  MonadWalletStatusView,
} from "./monad-views";
...
const registry: Record<string, StepView> = {
  ...
  ta_rsi: (p) => chart(twelveSeries(p, "rsi"), "RSI"),
  ta_sma: (p) => chart(twelveSeries(p, "sma"), "SMA"),
  ta_ema: (p) => chart(twelveSeries(p, "ema"), "EMA"),
  ta_macd: (p) => chart(twelveSeries(p, "macd"), "MACD"),
  ta_bbands: (p) => chart(twelveSeries(p, "middle_band"), "BBANDS (mid)"),
  ...
  tastedive_recommendations: (p, raw) => <GenericHumanView data={p} raw={raw} />,
  ...
  hacker_news_top_stories: (p) => <TopHeadlinesView data={p} />,
  ...
  // coinmarketcap (generated) — raw upstream {status, data} envelope; the
  // smart CmcView dispatches on the data shape (quote rows / cards / kv).
  altcoin_season_historical: (p) => <CmcView parsed={p} />,
  altcoin_season_latest: (p) => <CmcView parsed={p} />,
  api_key_info: (p) => <CmcView parsed={p} />,
  content_posts_comments: (p) => <CmcView parsed={p} />,
  content_posts_latest: (p) => <CmcView parsed={p} />,
  content_posts_top: (p) => <CmcView parsed={p} />,
  crypto_categories: (p) => <CmcView parsed={p} />,
  crypto_category: (p) => <CmcView parsed={p} />,
  crypto_info: (p) => <CmcView parsed={p} />,
  crypto_listings_historical: (p) => <CmcView parsed={p} />,
  crypto_listings_latest: (p) => <CmcView parsed={p} />,
  crypto_map: (p) => <CmcView parsed={p} />,
  crypto_multiplier: (p) => <CmcView parsed={p} />,
  crypto_quotes_historical: (p) => <CmcView parsed={p} />,
  crypto_quotes_latest: (p) => <CmcView parsed={p} />,
  derivatives_liquidations_crypto_latest: (p) => <CmcView parsed={p} />,
  derivatives_liquidations_exchange_latest: (p) => <CmcView parsed={p} />,
  derivatives_liquidations_quotes_latest: (p) => <CmcView parsed={p} />,
  derivatives_market_pairs_latest: (p) => <CmcView parsed={p} />,
  dex_holder_count: (p) => <CmcView parsed={p} />,
  dex_holder_detail: (p) => <CmcView parsed={p} />,
  dex_holders: (p) => <CmcView parsed={p} />,
  dex_liquidity_change_list: (p) => <CmcView parsed={p} />,
  dex_listings_info: (p) => <CmcView parsed={p} />,
  dex_listings_quotes: (p) => <CmcView parsed={p} />,
  dex_meme_list: (p) => <CmcView parsed={p} />,
  dex_multiplier: (p) => <CmcView parsed={p} />,
  dex_networks: (p) => <CmcView parsed={p} />,
  dex_pairs_quotes_latest: (p) => <CmcView parsed={p} />,
  dex_pairs_trade_latest: (p) => <CmcView parsed={p} />,
  dex_platform_detail: (p) => <CmcView parsed={p} />,
  dex_platform_list: (p) => <CmcView parsed={p} />,
  dex_search: (p) => <CmcView parsed={p} />,
  dex_security_detail: (p) => <CmcView parsed={p} />,
  dex_spot_pairs_latest: (p) => <CmcView parsed={p} />,
  dex_token_info: (p) => <CmcView parsed={p} />,
  dex_token_liquidity: (p) => <CmcView parsed={p} />,
  dex_token_pools: (p) => <CmcView parsed={p} />,
  dex_token_price: (p) => <CmcView parsed={p} />,
  dex_token_transactions: (p) => <CmcView parsed={p} />,
  exchange_assets: (p) => <CmcView parsed={p} />,
  exchange_derivatives_list: (p) => <CmcView parsed={p} />,
  exchange_derivatives_market_pairs_latest: (p) => <CmcView parsed={p} />,
  exchange_info: (p) => <CmcView parsed={p} />,
  exchange_map: (p) => <CmcView parsed={p} />,
  exchange_quotes_historical: (p) => <CmcView parsed={p} />,
  fear_and_greed_historical: (p) => <CmcFearGreedView parsed={p} />,
  fear_and_greed_latest: (p) => <CmcFearGreedView parsed={p} />,
  fiat_map: (p) => <CmcView parsed={p} />,
  flipside_fcas_listings_latest: (p) => <CmcView parsed={p} />,
  flipside_fcas_quotes_latest: (p) => <CmcView parsed={p} />,
  global_metrics_quotes_historical: (p) => <CmcView parsed={p} />,
  global_metrics_quotes_latest: (p) => <CmcView parsed={p} />,
  index_cmc100_historical: (p) => <CmcView parsed={p} />,
  index_cmc100_latest: (p) => <CmcView parsed={p} />,
  index_cmc20_historical: (p) => <CmcView parsed={p} />,
  index_cmc20_latest: (p) => <CmcView parsed={p} />,
  kline_candles: (p) => <CmcOhlcvView parsed={p} />,
  kline_points: (p) => <CmcOhlcvView parsed={p} />,
  dex_pairs_ohlcv_historical: (p) => <CmcOhlcvView parsed={p} />,
  dex_pairs_ohlcv_latest: (p) => <CmcOhlcvView parsed={p} />,
  postman_collection: (p) => <CmcView parsed={p} />,
  price_conversion: (p) => <CmcView parsed={p} />,
  rwa_assets_list: (p) => <CmcView parsed={p} />,
  rwa_info: (p) => <CmcView parsed={p} />,
  rwa_issuers: (p) => <CmcView parsed={p} />,
  rwa_issuers_list: (p) => <CmcView parsed={p} />,
  rwa_map: (p) => <CmcView parsed={p} />,
  rwa_quotes_latest: (p) => <CmcView parsed={p} />,
  simple_price: (p) => <CmcView parsed={p} />,

  // monad agent tools — camelCase structs + `chain` stamp
  monad_wallet_status: (p) => (isRecord(p) ? <MonadWalletStatusView data={p} /> : null),
  monad_token_balance: (p) => (isRecord(p) ? <MonadTokenBalanceView data={p} /> : null),
  monad_allowance: (p) => (isRecord(p) ? <MonadTokenBalanceView data={p} /> : null),
  monad_token_info: (p) => (isRecord(p) ? <MonadTokenInfoView data={p} /> : null),
  monad_gas_price: (p) => (isRecord(p) ? <MonadGasView data={p} /> : null),
  monad_chain_status: (p) => (isRecord(p) ? <MonadChainStatusView data={p} /> : null),
  monad_tx_receipt: (p) => (isRecord(p) ? <MonadTxReceiptView data={p} /> : null),
  monad_logs: (p) => (isRecord(p) ? <MonadLogsView data={p} /> : null),

  // analytics — export result is a stored file ({fileId, filename, format,
  // rows, bytes}) → the file-created card opens it in the preview
  data_export: (p) => (isRecord(p) ? <FileCreatedView data={p} /> : null),
};
