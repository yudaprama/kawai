...
function extractOfficeFile(value: unknown): { id: string; name: string; bytes?: number } | null {
  ...
  const name = typeof file.originalName === "string" ? file.originalName : typeof file.filename === "string" ? file.filename : null;
  if (!id || !name) return null;
  const bytes = typeof file.bytes === "number" ? file.bytes : undefined;
  return { id, name, bytes };
}

export function ToolWorkbench({ messages, toolCallId, onBack, onOpenPreview, onOpenCodeGraph }: { messages: UIMessage[]; toolCallId: string; onBack: () => void; onOpenPreview?: (fileId: string, name: string) => void; onOpenCodeGraph?: (query: string, result: string) => void }) {
  ...
  const codegraphQuery = selected?.input && typeof selected.input === "object" && "query" in selected.input && typeof selected.input.query === "string" ? selected.input.query : "";
          ...
          <div className="mx-auto w-full max-w-3xl space-y-4">
            ...
            {name === "codegraph_explore" && typeof codegraphResult === "string" ? (
              <div className="space-y-3">
                <pre className="bg-muted max-h-[70vh] overflow-auto whitespace-pre-wrap rounded-md p-4 text-xs leading-relaxed">{codegraphResult}</pre>
                {onOpenCodeGraph && <button className="text-primary hover:underline text-xs" onClick={() => onOpenCodeGraph(codegraphQuery, codegraphResult)} type="button">Open in CodeGraph</button>}
              </div>
            ) : officeFile ? (
              <div className="space-y-3">
                ...
                {onOpenPreview && (
                  <button className="text-primary inline-flex items-center gap-1 hover:underline text-xs" onClick={() => onOpenPreview(officeFile.id, officeFile.name)} type="button">
