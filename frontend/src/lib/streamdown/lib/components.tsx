...
type ElementLike = { children?: React.ReactNode; [key: string]: unknown };
const elementProps = (
  el: React.ReactElement
): ElementLike => el.props as ElementLike;
  ...
  ({ children, className, node, ...props }: SectionProps) => {
    ...
    if (isFootnotesSection) {
      ...
      const isEmptyFootnote = (listItem: React.ReactNode): boolean => {
        ...
        const lp = elementProps(listItem);
        const itemChildren = (
          Array.isArray(lp.children) ? lp.children : [lp.children]
        ) as React.ReactNode[];
        ...
        for (const itemChild of itemChildren) {
          ...
          if (typeof itemChild === "string") {
            ...
            if (elementProps(itemChild)["data-footnote-backref"] !== undefined) {
              hasBackref = true;
            } else {
              // It's some other element (like <p>), which means it has content
              // But we need to check if the <p> has actual content
              const ip = elementProps(itemChild);
              const grandChildren = (
                Array.isArray(ip.children) ? ip.children : [ip.children]
              ) as React.ReactNode[];
              ...
              for (const grandChild of grandChildren) {
                ...
                if (
                  ...
                  elementProps(grandChild)["data-footnote-backref"] === undefined
                ) {
      ...
      const processedChildren = Array.isArray(children)
        ? children.map((child) => {
            ...
            if (child.type === MemoOl) {
              const cp = elementProps(child);
              const listChildren = (
                Array.isArray(cp.children) ? cp.children : [cp.children]
              ) as React.ReactNode[];
