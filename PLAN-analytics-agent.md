...
`rig-components/analytics` crate (30 unit tests green), the kawai wiring
...
|---|---|---|
| New crate | `rig-components/analytics` | binance/webread pattern: hand-written `PortableTool`s, transport-agnostic, zero kawai deps. All Polars knowledge lives here, unit-testable with fixture files. |
...
### Phase 1 — `rig-components/analytics` crate (no kawai wiring)
...
- `rig-components/analytics/src/engine.rs` — AST → LazyFrame translation,
  literal coercion, schema validation (the only file that touches polars
  exprs).
- Add `"analytics"` to `rig-components/Cargo.toml` workspace members.
...
Files:
- `src-tauri/Cargo.toml` — `analytics = { path = "../rig-components/analytics", optional = true }`;
...
- **SQL sources → Parquet snapshot** (external Postgres/MySQL; local SQLite
  ...
  - **Credentials follow the binance pattern**
    (`rig-components/binance/src/account.rs`): named profiles live in env
