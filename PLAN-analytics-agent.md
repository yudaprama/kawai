...
`crates/engines/analytics` crate (30 unit tests green), the kawai wiring
...
(the LiteRT embedder fallback is `#[cfg]`-gated out on Android/iOS; remote embedding
...
|---|---|---|
| New crate | `crates/engines/analytics` | binance/webread pattern: hand-written `PortableTool`s, transport-agnostic, zero kawai deps. All Polars knowledge lives here, unit-testable with fixture files. |
...
### Phase 1 — `crates/engines/analytics` crate (no kawai wiring)

Files:
- `crates/engines/analytics/Cargo.toml` — deps: `polars 0.55` (features
  above), `serde`, `serde_json`; dev-deps `tokio`. Lints mirror binance
  (`unexpected_cfgs = warn`).
- `crates/engines/analytics/src/lib.rs` — `ToolError`/`terr` + public fns:
...
- `crates/engines/analytics/src/engine.rs` — AST → LazyFrame translation,
...
Files:
- `src-tauri/Cargo.toml` — `analytics = { path = "../crates/engines/analytics", optional = true }`;
...
- **SQL sources → Parquet snapshot** (external Postgres/MySQL; local SQLite
  ...
  - **Credentials follow the binance pattern**
    (`crates/toolsets/binance/src/account.rs`): named profiles live in env
