...
tool on the office agent, `crates/webread/src/scrape.rs` engine chain + budgets + cache,
...
Decision context (2026-08-23): the existing `crates/tools/browser`
...
```rust
// crates/webread/src/scrape.rs  (PURE — no tauri/axum)
...
`crates/webread/src/scrape.rs` (pure), so both engines are judged by the same rule.
...
This adds `browser = { path = "../crates/tools/browser" }` to
`src-tauri/Cargo.toml` (pure reqwest+rustls deps — mobile/web checks safe).

### 3.5 The tool

```rust
// crates/webread/src/scrape.rs
...
1. ~~Delete `crates/tools/browser/src/fallback.rs`~~ — **kept**: the
...
| `crates/webread/src/scrape.rs` | NEW — trait, engine chain, detection, cache, budgets, `WebReadTool` (+ unit tests) |
...
| `crates/tools/browser` | unchanged |
